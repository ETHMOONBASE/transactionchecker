...
function updateGame() {
    if(game.state == "STARTING") {
        ...
        $("#right").css("color", "#000");
        $("#midBar.result").css("border-bottom", "9px solid #BDC3C7");
        var percentage = ((100*((Math.floor(100 * Math.pow(Math.E, 0.00006 * game.elapsed)))/100))/goal);
        var size = (270 * percentage)/100;
        if(size > 270) size = 270;
        $("#midBar.result").css("width", size+"px");
        $("#gameBust").text(((Math.floor(100 * Math.pow(Math.E, 0.00006 * game.elapsed)))/100) + "x");
        if(((Math.floor(100 * Math.pow(Math.E, 0.00006 * game.elapsed)))/100) >= goal){
